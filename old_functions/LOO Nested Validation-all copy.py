...
def rfe_and_svd(args):
    ...
    train_df['Replicate'] = [f.split('_')[1] for f in train_df['Filename'].values]
    test_df['Replicate'] = [f.split('_')[1] for f in test_df['Filename'].values]
    feats = ['Replicate','Replicates',truth_feature] + list(feats)
    if len(var_cutoffs) == 1:
        return mats, {'Test Replicate': test_reps, 'Truth': truth_feature, 'Train': train_df[feats], 'Test': test_df[feats]}
    else:
        return mats
...
def evaluate_accuracies_varying_feature_and_variance(df, all_features, truth, acc_training_arrays = None, max_features=40, variance_cutoffs=None, urine = False, testing_mode = 'First'):
    ...
    if acc_training_arrays is None:
        ...
        batch = train_tasks[:]
        results = Parallel(n_jobs= 10, verbose=0, backend='loky')(delayed(rfe_and_svd)(task) for task in tqdm(batch))
    ...
    for test_rep, arr in acc_training_arrays.items():
        ...
        inds = sort_args[(arr>=np.max(arr)-0/len(acc_training_arrays)).ravel()]
        if np.sum(arr>=np.max(arr)-0/len(acc_training_arrays))>1:
            print(f'{test_rep} has multiple solutions ({np.max(arr):.2f})')
            degenerate_arrays[test_rep] = arr.copy()
        ...
        for n_ind, ind in enumerate(inds):
            ...
            if testing_mode in ['First']:
                ...
                break
    plt.figure()
    plot_all(degenerate_arrays, fullname=False, new_fig=False)
    
    accs = defaultdict(list) 
    results = Parallel(n_jobs= 10, verbose=0, backend='loky')(delayed(rfe_and_svd)(task) for task in tqdm(test_tasks))
    big_acc = {i: np.nan * np.zeros((len(variance_cutoffs_list), len(num_features_list))) for i in df['Replicates'].unique()}
    old_used_features = defaultdict(list)  # {test_rep: [used_features]}
    for task, (conf_mat,used_feats) in zip(test_tasks,results):
        ...
        old_used_features[test_rep].append(used_feats)
    ...
    if testing_mode == 'Max':
        inds = [np.argmax(np.trace(mat)/np.sum(mat)) for mats in accs.values() for mat in mats]
    elif testing_mode in ['First', 'Generalize']:
        inds = [0 for _ in accs.values()]
    
    old_accs = accs.copy()
    accs = []
    used_features = []
    for rep,ind in zip(old_accs.keys(),inds):
        accs.append(list(old_accs[rep])[ind])
        used_features.append(list(old_used_features[rep])[ind])
    
    

    return accs, used_var_cutoffs, used_num_features, acc_training_arrays, used_features
...
if __name__ == '__main__':
    ...
    for r,_,files in os.walk(feature_root):
        for file in files:
            if file!=f'features{feature_mode}.pickle': continue
            ...
            if 'ID' in r:
                ...
                this_feat_df = this_feat_df.loc[this_feat_df['Filename'].map(lambda x: t in x)].copy()
    ...
    all_used_features = defaultdict(dict)  # example -> {test: {family: {drug: features}}}
    for test,feat_df in allfeats.items():
        ...
        all_mats = [] 
        combined_group_df = defaultdict(list)  # (family, drug) -> list of DataFrames   
        for truth_feature in truth_features[test]:
            ...
            for group, this_df in tqdm(grouped,desc = f'Running {test}-{truth_feature}',leave=True):
                ...
                drugbug = '-'.join(group) + feature_mode
                ...
                if any([drugbug in file for file in os.listdir(path)]) and full_run:
                    ...
                    testing_mode = 'Generalize' # 'First' or 'Max' or 'Average' or 'Generalize'
                    accs, _, _, _, used_features = evaluate_accuracies_varying_feature_and_variance(this_df, all_feats, truth_feature, acc_training_arrays=training_arrs, urine = test=='Urine', testing_mode=testing_mode)
                    ...
                    accs, variances, nums, training_arrs, used_features = evaluate_accuracies_varying_feature_and_variance(this_df, all_feats, truth_feature, acc_training_arrays=training_arrs, urine = test=='Urine')
                    new_values = True
                ...
                plt.close(fig)
                all_mats.append([mat,sorted(this_df[truth_feature].unique())])
            
                mergers = ['Replicate','Replicates']
                this_df['Replicate'] = [file_name.split('_')[1] for file_name in this_df['Filename'].values]
                combined_group_df[group[0]].append(used_features)
            break


        combined_accs = {}
        for group, df_list in combined_group_df.items():
            if len(df_list) == 1: continue
            if test != 'ID': break
            mats = []
            for dic0, dic4 in tqdm(zip(*df_list),desc=f'Combining {group}',leave=True, total = len(df_list)):
                test_rep = dic0['Test Replicate']
                truth_feature = dic0['Truth']
                not_features = [truth_feature] + mergers
                df1 = dic0['Train']
                df2 = dic4['Train']
                combined_train_df = pd.merge(df1.reset_index(drop=True), df2.reset_index(drop=True), 
                  on=not_features, how="inner", suffixes=("_0","_4"))
                combined_train_df['Filename'] = np.arange(len(combined_train_df))

                df1 = dic0['Test']
                df2 = dic4['Test']
                combined_test_df = pd.merge(df1.reset_index(drop=True), df2.reset_index(drop=True), 
                  on=not_features, how="inner", suffixes=("_0","_4"))
                combined_test_df['Filename'] = np.arange(len(combined_test_df))
                print(combined_test_df)
                clf = ML(combined_train_df, [col for col in combined_train_df.columns if not any([feat in col for feat in not_features])], truth_feature, scale=True)
                clf.svm_test(combined_test_df)
                
                group_must_agree = False
                mat, _ = clf.make_confusion(all_must_agree = group_must_agree)
                mats.append(mat)
            mat = np.sum(mats,axis=0)
            print(mat/(2 if group_must_agree else 1))
            print(np.trace(mat)/(2 if group_must_agree else 1))
            print(np.sum(mat)/(2 if group_must_agree else 1))
            print(np.trace(mat)/np.sum(mat))
            print(len(mats), len(mats)/(np.sum(mat)/(2 if group_must_agree else 1)))
            plot_confusion(mat,labels=sorted(combined_train_df[group].unique()));plt.suptitle(group)
            combined_accs[group] = mat
        ...
        for ind, mat in combined_accs.items():
            acc = np.trace(mat) / np.sum(mat)
            tot = np.sum(mat)
            plt.bar(ind+'\nCombined', acc * 100, color='purple')
            bar_text = f'{acc * 100:.1f}\n{tot}'
            plt.text(ind+'\nCombined', acc * 100 - 1, bar_text, ha='center', va='top', color='white', fontsize=12)
        ...
        finalfig.savefig(os.path.join(fullpath, f'Bar Accuracies-{test}{feature_mode}.png'), dpi=300)



        plt.figure(f'Confusion Matrices-{test}-{truth_feature}', figsize=(13, 7))
        plt.suptitle(f'Confusion Matrices for {test} using Nested CV', fontsize=16)
        for n, (mat,labels) in enumerate(all_mats):
            ax = plt.subplot(2, int(np.ceil(len(all_mats)/2)), n+1)
            plot_confusion(mat, labels=labels,stats=False, ax = ax)
